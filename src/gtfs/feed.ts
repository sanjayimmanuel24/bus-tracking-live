...
async function fetchCsv(baseUrl: string, file: string): Promise<Record<string, string>[]> {
...
export async function loadFeed(baseUrl: string): Promise<TransitFeed> {
  const [agencyRows, stopRows, routeRows, tripRows, stopTimeRows, shapeRows] = await Promise.all([
    fetchCsv(baseUrl, 'agency.txt'),
    fetchCsv(baseUrl, 'stops.txt'),
    fetchCsv(baseUrl, 'routes.txt'),
    fetchCsv(baseUrl, 'trips.txt'),
    fetchCsv(baseUrl, 'stop_times.txt'),
    fetchCsv(baseUrl, 'shapes.txt'),
  ]);
