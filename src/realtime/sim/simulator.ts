...
export class SimulatedFeedSource implements FeedSource {
  ...
  private warmStart(): void {
    for (const bus of this.buses) {
      ...
      const distance = this.scheduledDistanceAt(trip, this.simSec);
      bus.distanceM = distance;

      let stopIndex = trip.stopTimes.findIndex((st) => st.shape_dist_traveled > distance);
      if (stopIndex < 0) stopIndex = trip.stopTimes.length - 1;
      bus.stopIndex = Math.max(1, stopIndex);
      bus.phase = 'running';
      bus.delaySec = 0;
  ...
  /**
   * Inverse of `scheduledTimeAt`: how far along the trip a bus should be at a
   * given time, in metres. Returns the stop's own distance while it is scheduled
   * to be dwelling there.
   */
  private scheduledDistanceAt(trip: TripIndex, timeSec: number): number {
    const st = trip.stopTimes;
    if (timeSec <= st[0]!.departure_time) return st[0]!.shape_dist_traveled;
    ...
    for (let i = 1; i < st.length; i++) {
      ...
      if (timeSec <= cur.arrival_time) {
        ...
        const frac = span <= 0 ? 1 : (timeSec - prev.departure_time) / span;
        return prev.shape_dist_traveled + frac * (cur.shape_dist_traveled - prev.shape_dist_traveled);
      }
    }
    return st[st.length - 1]!.shape_dist_traveled;
  }
  ...
  start(): void {
    ...
    this.timer = window.setInterval(() => this.frame(), 1000 / this.opts.snapshotHz);
    this.emit();
  }
  ...
  private stepBus(bus: SimBus, dt: number): void {
    ...
    switch (bus.phase) {
      ...
      case 'running': {
        ...
        this.updatePosition(bus, trip);
        bus.delaySec = this.simSec - this.scheduledTimeAt(trip, bus.distanceM);
        return;
      }
  ...
  /**
   * The time the timetable says a bus should be at a given distance along the trip.
   * Interpolates linearly within the segment, which matches how the segment times
   * were derived in the first place.
   */
  private scheduledTimeAt(trip: TripIndex, distanceM: number): number {
    const st = trip.stopTimes;
    if (distanceM <= st[0]!.shape_dist_traveled) return st[0]!.departure_time;
    ...
    for (let i = 1; i < st.length; i++) {
      ...
      if (distanceM <= cur.shape_dist_traveled) {
        ...
        const frac = span <= 0 ? 1 : (distanceM - prev.shape_dist_traveled) / span;
        return prev.departure_time + frac * (cur.arrival_time - prev.departure_time);
      }
    }
    return st[st.length - 1]!.arrival_time;
  }
